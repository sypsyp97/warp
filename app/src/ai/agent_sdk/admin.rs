...
/// Slim fork: there is no Warp login flow, so this command just explains the
/// situation and exits. The subcommand stays wired so that scripts/users
/// invoking `warp login` get a helpful message instead of a silent no-op.
pub fn login(ctx: &mut AppContext) -> Result<()> {
    println!(
        "This Warp build has no Warp account / login flow. \
         Configure your own LLM provider via Settings -> AI -> BYO LLM Provider, \
         or set the WARP_BYO_* environment variables."
    );
