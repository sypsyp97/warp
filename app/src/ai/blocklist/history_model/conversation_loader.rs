...
/// Stub for the upstream cloud conversation fetcher.
///
/// Slim has no Warp backend, so this always returns None. The signature is
/// preserved so the dispatch sites in `load_conversation_data` and the future
/// boxing helpers below stay unchanged.
