...
impl RootView {
    ...
    fn handle_agent_onboarding_event(
        ...
        match event {
            ...
            AgentOnboardingEvent::OnboardingCompleted(selected_settings) => {
                ...
                // Slim fork: BYO key replaces Warp accounts entirely, so
                // we never gate onboarding on login and skip the
                // post-onboarding LoginSlide unconditionally.
            ...
            AgentOnboardingEvent::UpgradePasteTokenFromClipboardRequested => {
                ...
                // the cloud login flow, so this onboarding event is a no-op.
            }
            AgentOnboardingEvent::PrivacySettingsFromTerminalThemeSlideRequested => {
                // Slim fork: the post-onboarding LoginSlide is gone. Skip
                // straight to the workspace via the same path the slide
                // would have eventually taken.
            ...
            AgentOnboardingEvent::LoginFromWelcomeRequested => {
                // Slim fork: there is no Warp account to log into, so the
                // "log in" link from the welcome slide just completes
                // onboarding and drops the user into the workspace.
                self.auth_onboarding_state
                    .complete_auth_and_create_workspace(ctx);
                ctx.emit(RootViewEvent::AuthOnboardingStateChanged);
                self.focus(ctx);
    ...
    /// finished onboarding pre-login and has since authenticated.
    ///
    /// Slim fork: there is no live AuthComplete event in the slim fork, so this
    /// helper is currently exercised only by the `root_view_tests` module. Kept
    /// behind `#[cfg(test)]` so the lib build stays warning-free.
    #[cfg(test)]
