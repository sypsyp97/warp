...
impl TerminalView {
    ...
    pub(in crate::terminal::view) fn show_out_of_credits_modal(&self, ctx: &mut ViewContext<Self>) {
        // Slim fork: the cloud-agent capacity modal can never open here (it required
        // server-side billing state). Just refresh AI request usage for the
        // non-paid-plan path, which is the only path slim ever takes.
